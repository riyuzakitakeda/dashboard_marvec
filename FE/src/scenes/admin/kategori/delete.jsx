...
const DeleteKategori = ({ id, execute }) => {
    const { token } = useAuth();
    const [open, setOpenDialog] = useState(false);
    const handleOpenDialog = () => setOpenDialog(true);
    const handleCloseDialog = () => setOpenDialog(false);
    const handleConfirmation = () => deleteDataKategori(id);

    const theme = useTheme();
    const colors = tokens(theme.palette.mode);

    const deleteDataKategori = useCallback((keyid) => {
        fetch(process.env.REACT_APP_API_URL + "api/kategori/" + keyid, {
            method: 'delete',
